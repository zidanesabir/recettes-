const API_URL = import.meta.env.VITE_API_URL || 'http://198.168.1.167:5000/api';
...
export const recipesAPI = {
  ...
  getMyRecipes: async (params?: {
    ...
    if (params?.limit) queryParams.append('limit', params.limit.toString());
    const query = queryParams.toString();
    return request(`/recipes/my-recipes${query ? `?${query}` : ''}`);
  },
};
...
export const commentsAPI = {
  getByRecipe: async (recipeId: string) => {
    return request(`/comments/recipe/${recipeId}`);
  },
